...
function ChangeLinr(settings) {
    ...
    var self = this, transforms, pipeline, pipelineLength, cache, cacheFull, doMakeCache, doUseCache, doUseGlobals;
    ...
    */
    self.reset = function (settings) {
        ...
        doMakeCache = typeof settings.doMakeCache === "undefined" ? true : settings.doMakeCache;

        doUseCache = typeof settings.doUseCache === "undefined" ? true : settings.doUseCache;

        doUseGlobals = settings.hasOwnProperty("doUseGlobals") ? false : settings.doUseGlobals;
        ...
        for (i = 0; i < pipelineLength; ++i) {
            ...
            if (!transforms.hasOwnProperty(pipeline[i])) {
                ...
                if (!transforms.hasOwnProperty(pipeline[i])) {
                    throw new Error("Pipe[" + i + "] (" + pipeline[i] + ") " + "not found in transforms.");
                }
            }

            // Also make sure each part of the pipeline is a function
            if (!(transforms[pipeline[i]] instanceof Function)) {
                throw new Error("Pipe[" + i + "] (" + pipeline[i] + ") " + "is not a valid function from transforms.");
            }
